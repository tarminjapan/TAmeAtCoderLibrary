...
public static partial class MathEx // partial は削除 (単一ファイルの場合)
{
    ...
    public static long Sqrt(long number)
    {
        ...
        int shift = BitOperations.Log2((ulong)number) & ~1;
        long bit = 1L << shift;
    ...
    /// 計算結果は常に 0 以上 modulus 未満の範囲で返されます。
    /// </summary>
    /// <param name="baseValue">底。</param>
    /// <param name="exponent">指数（負の値も対応）。</param>
    /// <param name="modulus">正の除数。</param>
    /// <returns>(baseValue ^ exponent) % modulus の結果（0 以上 modulus 未満）。</returns>
    /// <exception cref="ArgumentOutOfRangeException">modulus が 1 未満の場合。</exception>
    /// <exception cref="ArithmeticException">負の指数で、かつモジュラ逆数が存在しない場合。</exception>
    /// <remarks>
    /// 負の指数の場合はモジュラ逆数を使用して計算します。
    /// 例: ModPow(2, -3, 5) = ModInverse(2, 5) ^ 3 mod 5 = 3 ^ 3 mod 5 = 27 mod 5 = 2
    /// </remarks>
    public static long ModPow(long baseValue, long exponent, long modulus)
    {
        if (modulus <= 0)
            throw new ArgumentOutOfRangeException(nameof(modulus), "除数は正である必要があります。");

        // 法が 1 の場合、すべての数値は 0 と合同
        if (modulus == 1)
            return 0L;
        ...
        if (exponent < 0)
        {
            return ModPowWithNegativeExponent(baseValue, exponent, modulus);
        }

        // 非負の指数の場合、繰り返し二乗法を適用
        return ModPowNonNegative(baseValue, exponent, modulus);
    }

    /// <summary>
    /// 負の指数を持つべき乗剰余を計算します。
    /// </summary>
    private static long ModPowWithNegativeExponent(long baseValue, long exponent, long modulus)
    {
        ...
        if (inverseBase == -1)
        {
            throw new ArithmeticException(
                $"モジュラ逆数 {baseValue} mod {modulus} が存在しないため、負の指数を計算できません。");
        }

        // long.MinValue の場合、-exponent はオーバーフローするため特別に処理
        if (exponent == long.MinValue)
        {
            // (base^-1)^long.MaxValue * (base^-1) mod modulus を計算
            // 2 * long.MaxValue + 2 = 0 (mod 2^64) であるため、この方法で正確に計算できる
            long part1 = ModPowNonNegative(inverseBase, long.MaxValue, modulus);
            return part1 * inverseBase % modulus;
        }

        return ModPowNonNegative(inverseBase, -exponent, modulus);
    }

    /// <summary>
    /// 非負の指数を持つべき乗剰余を計算します（繰り返し二乗法）。
    /// </summary>
    private static long ModPowNonNegative(long baseValue, long exponent, long modulus)
    {
        // 底を [0, modulus - 1] の範囲に正規化（負の値も対応）
        long currentBase = ((baseValue % modulus) + modulus) % modulus;
        long result = 1L;

        while (exponent > 0)
        {
            // 指数の最下位ビットが 1 の場合、結果に現在の底を掛ける
            if ((exponent & 1) == 1)
            {
    ...
    public static long ModInverse(long number, long modulus)
    {
        ...
        var (g, x, _) = ExtendedGcd(numNormalized, modulus);
    ...
    public class ModularArithmetic
    {
        ...
        public long Inverse(long number)
        {
            ...
            return _inverseCache.GetOrAdd(normalizedNum, num => ModInverse(num, Modulus));
        }
        ...
        public long Multiply(long a, long b)
        {
            ...
            return (long)((Int128)na * nb % Modulus);
        }

        /// <summary>
        /// 数のべき乗を計算し、法（Modulus）による剰余を返します。
        /// </summary>
        public long Power(long baseValue, long exponent) => ModPow(baseValue, exponent, Modulus);
